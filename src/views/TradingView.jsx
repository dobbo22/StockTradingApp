...
const TradingView = ({ cash, onTrade }) => {
  ...
  const fetchStockDetails = async (symbol) => {
    ...
    try {
      ...
      if (stockData && stockData.quoteResponse && stockData.quoteResponse.result && stockData.quoteResponse.result.length > 0) {
        ...
        // Adjust price if currency is GBp (pence)
        let price = yahooData.regularMarketPrice || yahooData.price || 0;
        ...
        const forwardDividend = yahooData.dividendRate 
          ? formatCurrency(yahooData.dividendRate, currency)
          : 'N/A';
        ...
        const parsedDetails = {
          ...
          name: stockName,
          currentPrice: price,
          currency: currency === 'GBp' ? 'GBP' : (currency || 'GBP'),
          ...
          previousClose: yahooData.regularMarketPreviousClose || price || 0,
          open: yahooData.regularMarketOpen || 0,
          epsTrailingTwelveMonths: yahooData.epsTrailingTwelveMonths || 0,
          epsForward: yahooData.epsForward || 0,
          sharesOutstanding: yahooData.sharesOutstanding || 0,
          bookValue: yahooData.bookValue || 0,
          fiftyDayAverage: yahooData.fiftyDayAverage || 0,
          twoHundredDayAverage: yahooData.twoHundredDayAverage || 0,
          priceToBook: yahooData.priceToBook || 0,
          image: companyImage || '',
  ...
  // Render stock details content based on active tab
  const renderStockDetailsContent = () => {
    ...
    switch (activeTab) {
      // Update the 'overview' case in the renderStockDetailsContent function
// Replace the existing case with this:
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        // Update the 'company' case in the renderStockDetailsContent function
// Replace the existing case with this:
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="p-3 bg-gray-50 rounded-md">
        ...
        {stockDetails.website && stockDetails.website !== 'N/A' && (
          <div className="mb-4">
            <p className="text-sm text-gray-500">Website</p>
            <a 
              href={typeof stockDetails.website === 'string' && stockDetails.website.startsWith('http') ? 
                    stockDetails.website : 
                    `https://${safeRender(stockDetails.website)}`} 
              target="_blank" 
              rel="noopener noreferrer"
              className="text-blue-600 hover:underline"
            >
              {safeRender(stockDetails.website)}
            </a>
          </div>
        )}
